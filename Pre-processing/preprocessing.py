...
class WSIDataset():
    ...
    def _load_slide(self):
        ...
        logger.info("slides: {}".format(slides))
        return slides
...
def slide_to_patches(data,
    ...
    if use_cache: # use_cache = True
        cache_dir = os.path.join(output_dir, slide_bn) # Busca si en ".slide_patch_cache" esta el slide leído
        if not os.path.exists(cache_dir): # Sino existe
            os.makedirs(cache_dir) # Crea la parpeta
        patches = _load_cache(cache_dir) # Si existe, llama a la función load cache con la dirección ".slide_patch_cache/**slide**"
        if len(patches) > 0: # Si existe más de un parche
            data['patches'] = patches # Agrega en data el parche
            return data # retorna el data actualizado con la información de los parches
        
    slide = OpenSlide(slide_name) # Lee la WSI

    width, height = slide.dimensions # Captura sus dimensiones (No se ocupa ?)
    pixel_ratio = 2**crop_level # Se saca el pixel ratio (el crop_level es 1, porq lo 2¹, raro ??)
    thumbnail = slide.get_thumbnail(slide.level_dimensions[6]) # Captura el nivel 6 de la WSI
    thumbnail = np.array(thumbnail) # Transforma la slide en formato array
    ...
    for idx in range(bounds_x, bounds_width, crop_size * pixel_ratio):
        ...
        for idy in range(bounds_y, bound_height, crop_size * pixel_ratio):
            ...
            if np.sum(patch_gray > 210)/(patch_gray.shape[0]*patch_gray.shape[1]) > 0.9: # 245
                continue
...
data = WSIDataset(slide_dir='Dataset') # Capturo la data
patches = [] # Para guardar los patches
graymap = [] # Para guardar los graymap

# Capturo los patches
print("len data infos", len(data._data_infos))
for i in range(len(data._data_infos)):
    patches.append(slide_to_patches(data._data_infos[i]))

# Captura los graymap
print("len patches", len(patches))  
for j in range(len(patches)):
    graymap.append(compute_graymap(patches[j]))

# Pa graficar
'''print("len graymap", len(graymap))
for k in range(len(graymap)):
    plot_data(graymap[k],True,True)'''

# Nombre del archivo CSV a crear
filename = 'data_info.csv'
selected_columns = ["slide_bn", "ihc","fish","patch_name"]

print(" Genereando archivo csv\n")
...
with open(filename, mode='w', newline='') as file:
    ...
    # Escribir los datos
    i = 0
    while i < len(graymap): # POR CADA WSI
        j = 0
        while j < len(graymap[i]["patches"]): # POR CADA PARCHE
            name = graymap[i]["patches"][j]["patch_name"] # Capturo el nombre del parche
            name = name[27:] # Quito la ruta
            name, extension = os.path.splitext(name) # Quito la extensión
            writer.writerow([graymap[i]["slide_bn"],int(graymap[i]["lbls"]["ihc"]),int(graymap[i]["lbls"]["fish"]),name]) # Escribo la fila
            j = j + 1
        i = i + 1
print(" Archivo csv creado\n")
'''
with open("patches.txt", "w") as f:
    f.write(str(patches["patches"])) # Lista de diccionario: [{patch_name','crop_level', 'idx', 'idy','crop_size', 'patch'},...]

with open("graymap.txt", "w") as f:
    f.write(str(graymap)) # Diccionario: {'slide_fn':ruta del WSI', 'slide_bn':nombre del WSI', 'annos':weas del qupath, 'lbls':etiquetas, 'patches':lista de los parches generados}

with open("graymap_for_patch.txt", "w") as f:
    f.write(str(graymap["patches"]))
'''
print(" Generar archivos pkl\n")
# Genero los archivos pkl por cada parche
i = 0
while i < len(graymap): # POR CADA WSI
    num_patches = len(graymap[i]["patches"]) # Capturo el número de parches por WSI
    for j in range(num_patches): # Por cada parche
        # Crear un archivo pickle
        nombre_archivo, extension = os.path.splitext(graymap[i]["patches"][j]["patch_name"]) # Capturo el nombre del parche
        nueva_extension = ".pkl" # Agrego la extensión
        nuevo_nombre = nombre_archivo + nueva_extension # Genero el nombre del parche con la extensión
        with open(nuevo_nombre, 'wb') as f:
            pickle.dump(graymap[i]["patches"][j]["patch"], f) # Creo el archivo .pkl
    i = i + 1
